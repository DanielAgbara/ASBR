function [q, q_history, norm_err_hist] = constrained_linear_ls_pos_and_ori( ...
...
for k = 1:opts.max_iters
    ...
    C_pos = -skew(t)*J_alpha + J_epsilon;
    d_pos = p_goal - t;
    
    C_ori = -skew(R(:,3)) * J_alpha;
    d_ori = zeros(3,1);
    ...
    C = [sqrt(opts.w_dist) * C_pos;
         sqrt(opts.w_ori)  * C_ori];
    
    d = [sqrt(opts.w_dist) * d_pos;
         sqrt(opts.w_ori)  * d_ori];
