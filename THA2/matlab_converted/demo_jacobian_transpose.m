%% =========================
%  PA_h_make_videos.m
%  Simple pose IK test using Jacobian Transpose Pose IK
%% =========================
clear; clc; close all;
...
%% =========================
urdfPath = fullfile(fileparts(which('demo_jacobian_transpose')), '..', 'kuka_lbr_iiwa_support', 'urdf', 'lbr_iiwa_14_r820.urdf');
...
% Convert to body-frame screw axes
...
% Desired target pose
%% =========================
T_init = body_product_of_exponentials(M_ee, B_list, theta_a);
R_init = T_init(1:3,1:3);
p_init = T_init(1:3,4);

R_sd = R_init * AxisAngleToR([0;1;0], deg2rad(8));   % 약 8도 회전
p_sd = p_init + [0.03; 0.02; -0.04];                 % 3~4 cm 이동
...
% Jacobian transpose gain
% Start conservative. If convergence is too slow, increase slightly.
K = diag([0.05 0.05 0.05 0.05 0.05 0.05]);

%% =========================
% Solve pose IK using Jacobian Transpose
%% =========================
[theta_sol, theta_history, norm_w_b_hist, norm_v_b_hist] = ik_jacobian_transpose_pose( ...
    M_ee, ...
    B_list, ...
    theta_a, ...
    T_sd, ...
...
% Check final error
%% =========================
T_final = body_product_of_exponentials(M_ee, B_list, theta_sol);
T_err   = inv_SE3(T_final) * T_sd;
Vb_err  = se3_log_to_vec(T_err);

fprintf('Final angular error norm  = %.6e\n', norm(Vb_err(1:3)));
fprintf('Final linear  error norm  = %.6e\n', norm(Vb_err(4:6)));
...
%% =========================
if export_videos
save_robot_motion_video(robot, theta_history, T_list, Jw_cell, Jv_cell, 'transpose_robot_motion.mp4');
% save_metrics_video(T_list, Jw_cell, Jv_cell, ...
%     norm_w_b_hist, norm_v_b_hist, ...
%     cond_w_hist, cond_v_hist, ...
%     iso_w_hist, iso_v_hist,...
%     manip_hist, ...
%     'transpose_metrics.mp4');
% 
